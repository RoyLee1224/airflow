...
- **Run manual (slower) checks:** `prek run --from-ref <target_branch> --stage manual --skip compile-ui-assets-dev --skip view-skill-eval --skip run-skill-eval-codex --skip run-skill-eval-opencode` (the skipped hooks start long-running local servers or provision opt-in agent runtime environments rather than run checks that complete)
...
5. Run manual (slower) checks
   (`prek run --from-ref <target_branch> --stage manual --skip compile-ui-assets-dev --skip view-skill-eval --skip run-skill-eval-codex --skip run-skill-eval-opencode`)
   and fix any failures. The skipped hooks start long-running local servers or provision the
   opt-in agent runtime environments rather than run checks that complete.
