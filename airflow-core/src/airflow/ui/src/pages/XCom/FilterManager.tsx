...
const FILTER_LABELS: Record<FilterType, string> = {
  dag_display_name_pattern: "DAG Name",
  key_pattern: "Key",
  run_id_pattern: "Run ID",
...
export const FilterManager = ({
  ...
  const updateFilter = useCallback(
    (id: string, value: string) => {
      setFilters((previous) => {
        const updatedFilters = previous.map((filter) => (filter.id === id ? { ...filter, value } : filter));

        const filtersRecord = updatedFilters.reduce(
          (accumulator, filter) => {
  ...
  const removeFilter = useCallback(
    (id: string) => {
      setFilters((previous) => {
        const updatedFilters = previous.filter((filter) => filter.id !== id);
  ...
  const resetFilters = useCallback(() => {
    setFilters([]);
    onFiltersChange(defaultInitialFilters);
  }, [onFiltersChange]);
