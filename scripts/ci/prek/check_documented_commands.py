...
def _validate_breeze(raw: str, registry: dict[str, BreezeCommand]) -> str | None:
    ...
    while index < len(words):
        ...
        if nxt not in registry:
            # Suggest descendant paths relative to the node, so a missing group
            # word surfaces as e.g. `testing providers-integration-tests`.
            descendants = {p[len(node) + 1 :] for p in registry if p.startswith(f"{node} ")}
            return f"`{word}` is not a subcommand of `{node}`{_suggest(word, descendants)}"
